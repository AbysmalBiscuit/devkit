...
fn a_relative_config_path_resolves_like_the_discovered_one() {
    ...
    let (discovered, _) = devkit_config::resolve(None, &proj, None, None, None).unwrap();
    assert_eq!(discovered.defaults.worktree_root, expected);

    let (explicit, prov) =
        devkit_config::resolve(Some(Path::new("devkit.toml")), &proj, None, None, None).unwrap();
    assert_eq!(
    ...
    let (from_env, _) = devkit_config::resolve(None, &proj, None, None, None).unwrap();
    assert_eq!(

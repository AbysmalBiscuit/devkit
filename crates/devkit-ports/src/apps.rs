...
/// Build the catalog: an app's path comes from its explicit `path` or is
/// inferred from doppler.yaml. Returns the catalog alongside the names it
/// skipped.
///
/// An app whose path can be resolved neither from config nor from doppler.yaml
/// is skipped rather than failing the whole catalog — a config may list apps
/// that aren't present in every checkout. Requesting such an app surfaces a
/// plain "unknown app" error at the call site. The skipped names are returned
/// rather than printed because a caller reading *another* worktree's config has
/// no business reporting that config's gaps on this terminal.
pub fn catalog(
    cfg: &Config,
...
"#;
    ...
    fn infers_path_from_doppler_yaml() {
        ...
        // `api` has no explicit `path`; it is inferred from the doppler.yaml
        // key.
    ...
    fn skips_apps_with_unresolvable_path() {
        // `api` has no `path` in the sample; without a doppler entry for it, it
        // is skipped rather than erroring the whole catalog, and named

...
fn a_missing_config_is_created_with_the_directive_first() {
    ...
    let body = std::fs::read_to_string(dir.path().join("devkit.toml")).unwrap();
    assert_eq!(
...
fn every_setting_in_the_starter_is_commented_out() {
    let dir = tempfile::tempdir().unwrap();
    assert!(init(dir.path(), &[]).status.success());
    let body = std::fs::read_to_string(dir.path().join("devkit.toml")).unwrap();
...
fn an_existing_config_keeps_its_content_and_gains_the_directive() {
    ...
    std::fs::write(dir.path().join("devkit.toml"), original).unwrap();
...
fn running_it_twice_does_not_stack_directives() {
    let dir = tempfile::tempdir().unwrap();
    init(dir.path(), &[]);
    let once = std::fs::read_to_string(dir.path().join("devkit.toml")).unwrap();

    let out = init(dir.path(), &[]);
    assert!(out.status.success());
    let twice = std::fs::read_to_string(dir.path().join("devkit.toml")).unwrap();
...
fn an_explicit_path_is_honored() {
    ...
    let out = init(dir.path(), &["nested/devkit.toml"]);
    assert!(out.status.success());

    let body = std::fs::read_to_string(dir.path().join("nested/devkit.toml")).unwrap();
    assert_eq!(body.lines().next().unwrap(), DIRECTIVE);
    assert!(
        !dir.path().join("devkit.toml").exists(),

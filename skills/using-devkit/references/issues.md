...
## `checkout-pr` — review someone else's work
...
No head-commit check here: this is the reviewer's command, run in a worktree `checkout-pr` built, where `HEAD` falls behind as soon as the author pushes again. `[BODY]` fills the `review_finish` template's `{{ input }}`.

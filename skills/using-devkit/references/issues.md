...
- A reused PR keeps its draft state; a `--draft`/`--ready` that contradicts it is reported as ignored, naming `issue pr ready` or `gh pr ready --undo`.
...
- `pr ready` on a PR that is already ready changes nothing and exits zero. A branch with no PR is an error naming `issue pr create`; a merged or closed PR is refused.

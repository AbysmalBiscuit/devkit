---
name: using-devkit
description: "Use when editing files in a checkout several agents or sessions share, when a write or shell command is denied naming another holder, when running a project build or verification command by hand, when starting local dev servers or allocating ports, when setting up issue worktrees or shipping them for review, and when writing or changing a `devkit.toml`: rule injection, context files, the command guard, tasks. Covers the devkit CLIs: `devkit`, `devrun`, `issue`, `lockm`, `portm`."
...
| `references/config.md` | writing or changing a `devkit.toml`: `[rules]` injection, `[[context.files]]`, `[harness]` command-guard rules, `[tasks]`, and checking the merged result |

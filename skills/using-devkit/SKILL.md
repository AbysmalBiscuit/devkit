...
`lock`: advisory file locks that let parallel sessions claim files before editing,
...
call picks it up automatically (the `lock` binary reads `$DEVKIT_SESSION`):
...
```sh
lock status                                  # who holds what right now
lock acquire src/auth/session.rs src/auth/mod.rs --note "refactoring auth session"
...
```sh
lock release src/auth/session.rs src/auth/mod.rs
lock release --all                            # or: drop everything you hold
```
...
`lock check <paths>` (read-only, takes no claim) to poll. **Never** use
`lock release --force` to take a path from a live holder — that defeats the entire
...
re-`acquire` to renew. `--ttl 0` means no expiry. `lock prune` drops expired/dead
locks.
...
| Command | Purpose |
|---|---|
| `lock acquire <paths…> [--note S] [--ttl SECS]` | Claim paths (all-or-nothing). Exit 1 if any is held. |
| `lock check <paths…>` | Read-only: would `acquire` succeed? No claim taken. |
| `lock release <paths…>` / `lock release --all` | Drop your claims. |
| `lock status` / `lock status --all` | Show held locks (this project / every project). |
| `lock prune` | Drop expired or dead-session locks. |

Add `--json` to `acquire`/`check`/`status` for machine-readable output. Run
`lock <cmd> --help` for full flags.

## Tool overview

| Binary | What it does |
|---|---|
| `lock` | Advisory file locks for parallel sessions — the collaboration tool above. |
| `portman` | Port registry: `alloc`/`release`/`status`/`prune` dev-server ports without collisions. |
...
| `devkit-portd` | Background daemon owning the port registry. Started automatically by `portman`/`devrun`; you rarely invoke it directly. |
...
- **Editing a shared file without acquiring it** → you may clobber another agent's
  in-flight work. Always `acquire` first.
...
- **Acquiring files one at a time** → partial holds and stalls. Claim everything a
  unit of work touches in a single `acquire`.

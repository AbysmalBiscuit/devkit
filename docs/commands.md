...
- **`task`**: run a canned `[tasks]` oneshot or sequence (no name lists them, with the args each reads). `--arg key=value` sets a template variable the task reads; see [configuration.md](configuration.md#tasks).
  String `run` entries render one argument each. `{ expand = "files | split(';')" }` inserts separate string arguments from an expression without `{{ }}`; for example, `--arg 'files=new file.txt;other.txt'` supplies two paths while preserving the space. `--dry-run` displays the expanded plan without executing it. See [argument expansion](configuration.md#expanding-task-arguments) for validation and command-guard rules.

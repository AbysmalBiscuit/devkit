...
- **`up`**: defaults to `--role issue` and allocates ports from the live registry at start time, so a worktree needs no reservation up front. `--supervise` hands the servers to `devkitd`, which restarts them on crash within the crash-loop budget. `--dry-run` prints the launch plan without starting anything. It skips the spawn and nothing else: under `--role baseline` the baseline is bootstrapped first, since the plan's ports and commands are rooted in that tree. A dry run against a fork point with no baseline yet therefore creates the baseline worktree, copies the `defaults.worktree_include` files into it, writes each app's `prep_files`, runs each app's `setup` commands, and fires the `after_worktree_create` hooks — the same install work a real run does.

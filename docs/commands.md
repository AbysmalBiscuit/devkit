...
- **`baseline prune`**: removes every baseline no worktree's `.devkit/issue.toml` names any more, one pass under the baseline directory's lock. A `registered` baseline goes through `git worktree remove`; an `orphaned` one is reclaimed as a plain directory, unless its git directory still resolves — that is somebody's checkout and it is left alone. A directory carrying no marker is named and left where it is: devkit cannot prove it created it. Refused (unless `--force`): a baseline with running servers, and one carrying modified *tracked* files, which is a person's edit and the only thing in a baseline a rebuild would not bring back — untracked prep files and installed dependencies are expected and never count. Also refused, with no `--force`: the directory the caller is standing in. One baseline's refusal does not abandon the sweep, and a run that refused any exits non-zero. `--dry-run` reaches the same verdict for every slot and stops before carrying it out, so what it reports is what a real run would take; it holds no per-slot lock, so it never waits on a bootstrap in progress.

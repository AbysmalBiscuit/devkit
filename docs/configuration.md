...
An entry may be a bare string or a table: `msg = { default = "wip", required = "agents" }`, or `ticket = { required = "always" }` with no default at all. `default` is the fallback used when nothing else supplies the name; `required` is `"always"`, `"humans"`, `"agents"`, or `"never"` (the default) and names who must pass the arg on the command line. This is a different thing from minijinja's `default` filter: the config key decides whether an arg is required, the filter does not, and `{{ msg | default("wip") }}` in a template still leaves `msg` required if nothing declares it. See [Tasks](#tasks) for how a task's own `required_args` combines with this, and for the floor that keeps an arg with no default required no matter what `required` says. Who counts as an agent for `required` is `DEVKIT_CALLER`, then caller detection; see Environment below.
